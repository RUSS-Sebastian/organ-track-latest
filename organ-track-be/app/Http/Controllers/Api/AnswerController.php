...
class AnswerController extends Controller
{
    ...
    public function getReportById($reportId)
    {
        ...
        Log::info("Fetching AI report", ['report_id' => $reportId, 'user_id' => $userId]);
        ...
        if (!$report) {
            Log::warning("Report not found", ['report_id' => $reportId, 'user_id' => $userId]);
            return response()->json(['message' => 'Report not found'], 404);
        }

        // Log full report for debugging
        Log::debug("Report found", ['report' => $report->toArray()]);

        // Log AI response separately
        Log::debug("AI response data", ['ai_response' => $report->ai_response]);
